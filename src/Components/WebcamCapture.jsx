...
const WebcamCapture = () => {
  ...
  const capturePhoto = () => {
    if (capturedImages.length < 4) {
      const context =
        canvasRefs[capturedImages.length].current.getContext("2d");
      context.drawImage(
        videoRef.current,
        0,
        0,
      ...
      const image =
        canvasRefs[capturedImages.length].current.toDataURL("image/png");
    ...
    <>
      ...
      <div className="webcam-capture-container">
        ...
        {Array.from({ length: 4 - capturedImages.length }).map((_, index) => (
          <canvas
            key={index}
            ref={canvasRefs[capturedImages.length + index]}
            style={{ display: "none" }}
          />
        ))}
        {selectedImage !== null && (
